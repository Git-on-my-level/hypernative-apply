...
export const initCommand = new Command()
    ...
    if (options.interactive && isDryRun) {
        ...
        const monitorType = await askQuestion('🔍 What do you want to monitor?\n   1) Treasury wallets\n   2) Smart contracts\n   3) Both\n   Choose (1-3): ');
        const notifyChannel = await askQuestion('📢 Notification preference?\n   1) Slack only\n   2) Email only\n   3) Both\n   Choose (1-3): ');
        log.info(`\n✅ Configuration:`);
        log.info(`   Project: ${projectName || 'Unnamed Project'}`);
        log.info(`   Monitoring: ${monitorType === '1' ? 'Treasury wallets' : monitorType === '2' ? 'Smart contracts' : 'Both'}`);
        log.info(`   Notifications: ${notifyChannel === '1' ? 'Slack' : notifyChannel === '2' ? 'Email' : 'Both'}`);
