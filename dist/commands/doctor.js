...
async function checkDependencies(_verbose) {
    try {
...
async function checkConfiguration(verbose) {
...
async function checkAuthentication(verbose) {
    try {
...
async function checkApiConnectivity(verbose) {
    try {
        const config = await loadConfig();
...
async function checkNetworkConnectivity(verbose) {
    try {
...
export const doctorCommand = new Command()
    ...
    if (jsonOutput) {
        ...
        if (hasFailures) {
            ...
            log.info('   • Authentication: Set HN_CLIENT_ID and HN_CLIENT_SECRET environment variables');
