...
export const log = {
    ...
    httpRequest: (method, url, duration, status, requestId) => {
        if (!globalFlags.debug)
            return;
        const meta = {
        ...
        if (globalFlags.json) {
            ...
            console.log(useColors ? chalk.gray('🌐') : '🌐', `${method} ${url} - ${timing}${statusText}${reqId}`);
        }
    },
    // Special method for rate limit info
    rateLimit: (limit, remaining, resetTime, requestId) => {
        if (!globalFlags.debug)
            return;
        const meta = {
            ...
            const color = percentage < 20
                ...
                : percentage < 50
                    ...
                    : useColors
                        ...
                        : (s) => s;
            const reqId = requestId ? ` [req:${requestId.substring(0, 8)}]` : '';
            console.log(useColors ? chalk.gray('📊') : '📊', `Rate limit: ${color(`${remaining}/${limit}`)} (resets at ${resetTime.toISOString()})${reqId}`);
        }
    },
};
// Update logger format when global flags are set
export function updateGlobalFlags(flags) {
