 ...
 */
function cleanForFingerprinting(obj) {
 ...
 */
export function generateFingerprint(config) {
 ...
 */
export function generateResourceFingerprints(resources, kind) {
    const fingerprints = {};
    for (const [name, config] of Object.entries(resources)) {
        try {
            ...
            throw new Error(`Failed to generate fingerprint for ${kind} '${name}': ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    return fingerprints;
}
 ...
 */
export function fingerprintsEqual(hash1, hash2) {
    return hash1 === hash2;
}
/**
...
export function isValidFingerprint(fingerprint) {
    return (typeof fingerprint === 'string' && fingerprint.length === 64 && /^[a-f0-9]+$/i.test(fingerprint));
}
/**
 ...
 */
export function getCanonicalJSON(config) {
