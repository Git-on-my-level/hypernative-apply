...
export class Planner {
     ...
     */
    async generatePlan(config, options = {}) {
    ...
    topologicalSort(nodes) {
        ...
        const visit = (nodeName, path = []) => {
            ...
            if (node) {
                ...
                node.depth = Math.max(0, ...node.dependencies.map((dep) => (nodes[dep] ? nodes[dep].depth + 1 : 0)));
                if (hasCycle) {
                    return true;
                }
            }
     ...
     */
    async detectResourceChanges(config, currentState, options) {
        const changes = [];
     ...
     */
    async analyzeResourceChange(name, desired, existingState, options) {
        if (!existingState) {
            // New resource
            return {
                kind: desired.kind,
        ...
        if (desired.kind === 'custom_agent') {
            requiresReplace = await this.requiresCustomAgentReplace(name, desired.config, existingState.remote_id);
            if (requiresReplace) {
        ...
        let fieldDiffs;
        let riskLevel = requiresReplace ? 'high' : 'medium';
     ...
     */
    getResourceDependencies(name, config, kind) {
        const dependencies = [];
     ...
     */
    assessChangeRisk(kind, fieldDiffs) {
        // High-risk changes
     ...
     */
    orderChangesByDependencies(changes, dependencyGraph) {
     ...
     */
    async detectDrift(_currentState, _options) {
     ...
     */
    generateWarnings(dependencyGraph, changes, driftResult) {
        const warnings = [];
     ...
     * Generate plan metadata
     */
    generatePlanMetadata(config, changes) {
     ...
     */
    generateContentHash(changes) {
        const contentString = JSON.stringify(changes
            .map((change) => ({
            name: change.name,
            kind: change.kind,
     ...
     */
    extractDependencyList(dependencyGraph) {
        const dependencies = [];
     ...
     */
    async requiresCustomAgentReplace(name, desiredConfig, remoteId) {
        ...
        try {
            ...
            const storedType = existingState.metadata.agent_type;
 ...
 */
export async function generateExecutionPlan(config, baseDir, options) {
 ...
 */
export async function createPlanFile(plan) {
