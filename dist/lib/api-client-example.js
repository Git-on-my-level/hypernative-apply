...
async function basicApiClientExample() {
    try {
        ...
        const alerts = await apiClient.get('/api/v2/alerts', {
            limit: 10,
            severity: 'high',
        });
        console.log('Fetched alerts:', alerts.items.length);
        // Make a POST request to create a watchlist
        const newWatchlist = await apiClient.post('/api/v2/watchlists', {
            name: 'Test Watchlist',
...
async function paginationExample() {
    try {
        ...
        const allWatchlists = await fetchAllPages(apiClient, '/api/v2/watchlists', {}, // No query parameters
        { pageSize: 50 } // Options
        );
        ...
        const result = await paginationHelper.fetchAll('/api/v2/custom-agents', {}, // Base query parameters
        {
            pageSize: 100,
...
async function rateLimitingExample() {
    try {
        ...
        const requests = Array.from({ length: 10 }, (_, i) => apiClient.get('/api/v2/alerts', { limit: 10, offset: i * 10 }));
...
//# sourceMappingURL=api-client-example.js.map