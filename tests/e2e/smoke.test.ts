...
describe.skipIf(!isE2E)('E2E Smoke Tests', () => {
  ...
  const createBasicConfig = () => {
    ...
    writeFileSync(join(channelsDir, 'e2e-test-slack.yaml'), `
...
`);
    ...
    writeFileSync(join(watchlistsDir, 'e2e-test-watchlist.yaml'), `
...
`);
    ...
    process.env.E2E_SLACK_WEBHOOK_URL = 'https://hooks.slack.com/services/T00000000/B00000000/XXXXXXXXXXXXXXXXXXXXXXXX';
  };
...
`);
  ...
  describe('Apply Command', () => {
    ...
    it('should update existing resources', () => {
      ...
      // Modify config
      const channelFile = join(testWorkspace, 'hypernative', 'notification-channels', 'e2e-test-slack.yaml');
      writeFileSync(channelFile, `
...
`);
  ...
  describe('Configuration Validation', () => {
    it('should validate cross-references between resources', () => {
      ...
      writeFileSync(join(watchlistsDir, 'bad-watchlist.yaml'), `
name: Bad Watchlist
...
`);
    ...
    it('should handle environment variable substitution', () => {
      ...
      writeFileSync(join(channelsDir, 'dynamic.yaml'), `
...
`);
    ...
    it('should handle missing environment variables', () => {
      ...
      writeFileSync(join(channelsDir, 'missing-env.yaml'), `
