...
export class LogRedactor {
  private static config: RedactionConfig = DEFAULT_CONFIG;
  ...
  static redact(data: any): any {
    ...
    if (typeof data === 'object') {
      ...
      for (const [key, value] of Object.entries(data)) {
        ...
        const isSensitiveField = LogRedactor.config.sensitiveFields.some((sensitiveField) =>
   ...
   */
  static partialRedact(str: string, visibleChars: number = 4): string {
